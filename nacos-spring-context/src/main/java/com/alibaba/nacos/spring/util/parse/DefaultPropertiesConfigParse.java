...
public class DefaultPropertiesConfigParse extends AbstractConfigParse {
		...
		@Override public Map<String, Object> parse(String configText) {
				OriginTrackedPropertiesLoader loader = new OriginTrackedPropertiesLoader(
						new ByteArrayResource(
								configText.getBytes(Charset.defaultCharset())));
				try {
		...
		@Override public String processType() {
		...
		class OriginTrackedPropertiesLoader {
						...
						try {
								...
								while (reader.read()) {
										...
										if (expandLists && key.endsWith("[]")) {
												...
												int index = 0;
												do {
														OriginTrackedValue value = loadValue(
																buffer, reader, true);
														put(result, key + "[" + (index++)
																+ "]", value);
				...
				private OriginTrackedValue loadValue(StringBuilder buffer,
						...
						while (!reader.isEndOfLine() && !(splitLists && reader
								.isListDelimiter())) {
								buffer.append(reader.getCharacter());
								reader.read();
						}
						TextResourceOrigin origin = new TextResourceOrigin(this.resource,
								location);
						return OriginTrackedValue.of(buffer.toString(), origin);
				}
				
				/**
				 * Reads characters from the source resource, taking care of skipping
				 * comments, handling multi-line values and tracking {@code '\'} escapes.
				 */
				private class CharacterReader implements Closeable {
						...
						public boolean isEndOfLine() {
								return this.character == -1 || (!this.escaped
										&& this.character == '\n');
						}
						...
						public Location getLocation() {
								return new Location(this.reader.getLineNumber(),
										this.columnNumber);
						}
		...
		public abstract static class Origin {
				...
				/**
				 * Find the {@link Origin} that an object originated from. Checks if the
				 * source object is an {@link OriginProvider} and also searches exception
				 * stacks.
				 *
		...
		public static class OriginTrackedValue implements OriginProvider {
				 ...
				 * value} and {@code origin}. If the source value implements {@link
				 * CharSequence} then so will the resulting {@link OriginTrackedValue}.
				 *
				 * @param value  the source value
				 * @param origin the origin
				 * @return an {@link OriginTrackedValue} or {@code null} if the source
				 * value was {@code null}.
				 */
				public static OriginTrackedValue of(Object value, Origin origin) {
						...
						if (value instanceof CharSequence) {
								return new OriginTrackedCharSequence((CharSequence) value,
										origin);
						}
		...
		public class TextResourceOrigin extends Origin {
				...
				@Override public int hashCode() {
						int result = 1;
						result =
								31 * result + ObjectUtils.nullSafeHashCode(this.resource);
						result =
								31 * result + ObjectUtils.nullSafeHashCode(this.location);
						return result;
				}
				...
				@Override public boolean equals(Object obj) {
						...
						if (obj instanceof TextResourceOrigin) {
								...
								boolean result = true;
								result = result && ObjectUtils
										.nullSafeEquals(this.resource, other.resource);
								result = result && ObjectUtils
										.nullSafeEquals(this.location, other.location);
								return result;
						}
				...
				@Override public String toString() {
						...
						result.append(this.resource != null
