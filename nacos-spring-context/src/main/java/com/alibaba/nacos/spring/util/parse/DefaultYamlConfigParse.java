...
public class DefaultYamlConfigParse extends AbstractConfigParse {
		...
		protected static boolean process(MatchCallback callback, Yaml yaml,
				String content) {
				int count = 0;
				...
				if (LOGGER.isDebugEnabled()) {
						LOGGER.debug(
								"Loaded " + count + " document" + (count > 1 ? "s" : "")
		...
		@SuppressWarnings("unchecked") protected static Map<String, Object> asMap(
				Object object) {
				...
				for (Map.Entry<String, Object> entry : source.entrySet()) {
						...
						if (value instanceof String) {
								...
								for (Object object : collection) {
										buildFlattenedMap(result, Collections
												.singletonMap("[" + (count++) + "]",
														object), key);
								}
						}
		...
		@Override public Map<String, Object> parse(String configText) {
				...
				process(new MatchCallback() {
						@Override public void process(Map<String, Object> map) {
								result.set(map);
						}
		...
		protected static class MapAppenderConstructor extends Constructor {
				...
				@Override protected Map<Object, Object> constructMapping(
						MappingNode node) {
						try {
