...
public class EventPublishingConfigService implements ConfigService, NacosServiceMetaData {
		...
		public EventPublishingConfigService(ConfigService configService,
				...
				this.applicationEventPublisher = new DeferredApplicationEventPublisher(
						context);
		...
		@Override public String getConfig(String dataId, String group, long timeoutMs)
				...
				try {
						...
						if (NacosException.SERVER_ERROR == e
								.getErrCode()) { // timeout error
								publishEvent(
										new NacosConfigTimeoutEvent(configService, dataId,
												group, timeoutMs, e.getErrMsg()));
						}
		...
		@Override public String getConfigAndSignListener(String dataId, String group,
				...
				Listener listenerAdapter = new DelegatingEventPublishingListener(
						configService, dataId, group, applicationEventPublisher, executor,
						listener);
		...
		/**
		 * Implementation of the new version of support for multiple configuration file
		 * type resolution.
		 *
		 * @param dataId   dataId
		 * @param group    group
		 ...
		 */
		public void addListener(String dataId, String group, String type,
				Listener listener) throws NacosException {
				Listener listenerAdapter = new DelegatingEventPublishingListener(
						configService, dataId, group, type, applicationEventPublisher,
						executor, listener);
		...
		@Override public void addListener(String dataId, String group, Listener listener)
				...
				publishEvent(new NacosConfigListenerRegisteredEvent(configService, dataId,
						group, listener, true));
		}
		...
		@Override public boolean publishConfig(String dataId, String group,
				...
				publishEvent(new NacosConfigPublishedEvent(configService, dataId, group,
						content, published));
				return published;
		}
		...
		@Override public boolean removeConfig(String dataId, String group)
				...
				publishEvent(new NacosConfigRemovedEvent(configService, dataId, group,
						removed));
				return removed;
		}
