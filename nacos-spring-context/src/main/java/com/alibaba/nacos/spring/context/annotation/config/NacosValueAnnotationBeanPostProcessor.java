...
public class NacosValueAnnotationBeanPostProcessor
		...
		@Override protected Object doGetInjectedBean(NacosValue annotation, Object bean,
		...
		@Override protected String buildInjectedObjectCacheKey(NacosValue annotation,
		...
		@Override public void onApplicationEvent(NacosConfigReceivedEvent event) {
				...
				ReflectionUtils.doWithFields(bean.getClass(),
						new ReflectionUtils.FieldCallback() {
								@Override public void doWith(Field field)
										...
										NacosValue annotation = getAnnotation(field,
												NacosValue.class);
										doWithAnnotation(beanName, bean, annotation,
												field.getModifiers(), null, field);
								}
						});
		}
				...
				ReflectionUtils.doWithMethods(bean.getClass(),
						new ReflectionUtils.MethodCallback() {
								@Override public void doWith(Method method)
										throws IllegalArgumentException {
										NacosValue annotation = getAnnotation(method,
												NacosValue.class);
										doWithAnnotation(beanName, bean, annotation,
		...
		private void doWithAnnotation(String beanName, Object bean, NacosValue annotation,
				...
				if (annotation != null) {
						...
						if (annotation.autoRefreshed()) {
								...
								NacosValueTarget nacosValueTarget = new NacosValueTarget(
										bean, beanName, method, field);
		...
		private String resolvePlaceholder(String placeholder) {
				...
				if (placeholder.length()
						<= PLACEHOLDER_PREFIX.length() + PLACEHOLDER_SUFFIX.length()) {
						return null;
				}
		...
		private void setMethod(NacosValueTarget nacosValueTarget, String propertyValue) {
				...
				try {
						...
						if (logger.isDebugEnabled()) {
								logger.debug(
										"Update value with {} (method) in {} (bean) with {}",
										method.getName(), nacosValueTarget.beanName,
										propertyValue);
						}
				}
				catch (Throwable e) {
						if (logger.isErrorEnabled()) {
								logger.error("Can't update value with " + method.getName()
										+ " (method) in " + nacosValueTarget.beanName
										+ " (bean)", e);
						}
				}
		}
		...
		private void setField(final NacosValueTarget nacosValueTarget,
				...
				try {
						...
						if (logger.isDebugEnabled()) {
								logger.debug("Update value of the {}"
												+ " (field) in {} (bean) with {}", fieldName,
										nacosValueTarget.beanName, propertyValue);
						}
				}
				catch (Throwable e) {
						if (logger.isErrorEnabled()) {
								logger.error("Can't update value of the " + fieldName
										+ " (field) in " + nacosValueTarget.beanName
										+ " (bean)", e);
						}
				}
		}
		...
		private static class NacosValueTarget {
				...
				NacosValueTarget(Object bean, String beanName, Method method,
						Field field) {
						this.bean = bean;
